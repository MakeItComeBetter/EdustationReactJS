...
const Messages = ({
  ...
  useEffect(() => {
    // setLastMessageIndex(currentMessages?.find((e) => e?.checked === true)?.id);
    setLastMessageIndex(currentMessages?.find((e) => e?.checked === true && e?.author === currentUser?.uid)?.id)
  ...
  const getUserById = (id) => {
    return currentRoom?.membersDetails?.find((e) => e?.uid === id);
  };
  ...
  return (
    <Grid container>
      <Grid item xs={12} className="msgs__header">
        <Grid item xs={8} className="msgs__header_first">
          <Link
            to={MESSENGER_PATH}
            replace
            className="msgs__icon"
            onClick={() => clearCurrentRoomData()}
          >
            <IconButton>
          ...
          <Avatar
            src={currentUserChatting?.photoURL}
            alt=""
            sx={{ width: 38, height: 38, margin: 1 }}
          />
      ...
      <Grid
        item
        xs={12}
        className="msgs__container"
        id="scrollableDiv"
        style={{
          height:
            currentMessages?.length > 0 ? window.innerHeight - 110 : "auto",
        }}
      >
        ...
        >
          {currentMessages && currentMessages.length > 0 ? (
            currentMessages?.map((v, i) => (
              <div key={i} className="msgs__block">
                {v?.author === currentUser.uid ? (
                  <div
                    className={
                      v?.id !== lastMessageIndex
                        ? "msgs__curUsrMsg"
                        : "msgs__curUsrMsgWthSnn"
                    }
                  >
                    ...
                    {v?.id === lastMessageIndex ? (
                      <Avatar
                        alt={getUserById(v?.author)?.displayName}
                        src={getUserById(v?.author)?.photoURL}
            ...
            >
              ...
              />
              <strong>
                <h3>{currentUserChatting?.displayName}</h3>
              </strong>
              {friends?.filter((e) => e?.uid === currentUserChatting?.uid)
                .length > 0 ? (
